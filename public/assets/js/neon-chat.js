...
;(function ($, window, undefined) {
    ...
    $.extend(neonChat, {
        ...
        init: function () {
            ...
            $chat.find("form#chat-file").find('input[name=mms]').change(function () {
            ...
            $chat.find("form#chat-file").submit(function (e) {
                ...
                if (neonChat.isOpen && neonChat.$current_user) {
                    ...
                    $.ajax({
                        url: Config.defaultURL + "/messages/send",
                        type: 'POST',
                        data: formData,
                        async: false,
                        success: function (data) {
                            neonChat.pushMessage(id, data.text, $chat.data('current-user') + ' (' + data.sender + ')', moment()
                                .format('YYYY-MM-DD H:mm:ss'), false, false, data.id);
            ...
            $chat.on('click', '#attach_file', function (e) {
                $(this).next("#chat-file").find('input[name=mms]').click();
            });
        ...
        open: function ($user_link) {
            ...
            if (user_status) {
                ...
                $conversation_header.find('img').attr('src', $user_link.data('avatar'));
            }
        ...
        submitMessage: function () // Submit whats on textarea
        {
            ...
            if (this.isOpen && this.$current_user) {
                ...
                var obj = this;
                $.post(Config.defaultURL + "/messages/send", {
                    receivers: id,
                    text: msg.replace(/<.*?>/g, '')
                }, function (data) {
                    obj.pushMessage(id, data.text, $chat.data('current-user') + ' (' + data.sender + ')', moment()
                        .format('YYYY-MM-DD H:mm:ss'), false, false, data.id);
            ...
            if (this.isOpen && this.$current_user) {
                ...
                BootstrapDialog.confirm({
                    ...
                    callback: function (result) {
                        if (result) {
                            $.post(Config.defaultURL + "/messages/archive", {
                                user_id: id
                            }, function (data) {
                                obj.removeUser(id);
                                obj.close();
        ...
        archiveMessage: function (id) {
            if (this.isOpen && this.$current_user) {
                var user_id = this.$current_user.uniqueId().attr('id');
                var obj = this;
                BootstrapDialog.confirm({
                    title: 'Action Confirmation',
                    message: 'Do you really want to perform this action?',
                    ...
                    callback: function (result) {
                        if (result) {
                            $.post(Config.defaultURL + "/messages/archive", {
                                ...
                                obj.chat_history[user_id].messages = $.map(obj.chat_history[user_id].messages, function (value, index) {
                                    if (value.id !== id) {
                                        return [value];
                                    }
                                });
                                obj.renderMessages(user_id);
                            }, "json");
                        }
                    }
                });
        ...
        editContact: function () {
            if (this.isOpen && this.$current_user) {
                var id = this.$current_user.uniqueId().attr('id');
                $.get(Config.defaultURL + "/messages/contact/" + id, function (data) {
        ...
        pushMessage: function (user_id, msg, from, time, fromOpponent, unread, id) {
            if (user_id && msg) {
                this.refreshUserIds();
                ...
                var result = $.grep(this.chat_history[user_id].messages, function (e) {
                    return e.id === id;
                });

                if (result.length === 0) {
                    if (this.chat_history[user_id].messages.length >= max_chat_history) {
                        this.chat_history[user_id].messages = this.chat_history[user_id].messages.reverse()
                                                                                        .slice(0, max_chat_history - 1)
                                                                                        .reverse();
                    }
                    this.chat_history[user_id].messages.push({
                        id: id,
                        message: msg,
                        from: from,
                        time: time,
                    ...
                    this.chat_history[user_id].messages = $.map(this.chat_history[user_id].messages, function (value, index) {
                        if (value.id === id) {
                            value.message = msg;
                        }
                        return [value];
            ...
            if (typeof this.chat_history[id] != 'undefined') {
                ...
                for (var i in this.chat_history[id].messages) {
                    var entry = this.chat_history[id].messages[i],
                        $entry = $('<li data-msg-id="' + entry.id + '"><span class="user"></span><a href="#" class="delete-message" title="Archive Message"><i class="entypo-bag"></i></a><p></p><span class="time"></span></li>'),
                        date = entry.time,
                        date_formated = date;
            ...
            $chat.find('.chat-group a').each(function (i, el) {
                ...
                if (history_container && history_container.length) {
                    $($(history_container)).find('> li').each(function (j, el2) {
                        ...
                        neonChat.pushMessage(id, message, from, time, fromOpponent, unread, msg_id)
                    });
                }
        ...
        countUnreadMessages: function (id) {
            ...
            if (counter > 0) {
                this.animateTitle("...You got " + counter + " new messages", Config.defaultTitle, 1000);
            }
        ...
        animateTitle: function (text, defaultText, interval) {
            var obj = this;
            this.titleTimer = setTimeout(function () {
                ...
                obj.animateTitle(text, defaultText, interval);
            }, interval);
        },
        ...
        resetUnreads: function (id) {
            ...
            var user = this.chat_history[id];

            $.post(Config.defaultURL + "/messages/read", {
                id: id
            }, function (data) {
            }, "json");
        ...
        addUser: function (group_id, display_name, status, prepend, user_id, avatar) {
            var $group = group_id;
            ...
            if ($group && $group.length) {
                var status = this.statuses[status],
                    $user = $('<a href="#" data-avatar="' + avatar + '"><span class="user-status ' + status.class + '"></span> <em>' + display_name + '</em><span class="badge badge-success is-hidden">0</span></a>');

                if (user_id) {
                    $user.attr('id', user_id);
                }

                if (neonChat.getUser(user_id) !== null) {
                    neonChat.moveUser(user_id, group_id, true);
                    return null;
                }
    ...
    $conversation_body.on('mouseenter', 'li', function () {
        $(this).find(".delete-message").fadeIn();
    }).on('mouseleave', 'li', function () {
        $(this).find(".delete-message").fadeOut();
    }).on('click', '.delete-message', function () {
        neonChat.archiveMessage($(this).closest('li').data('msg-id'));
    });
